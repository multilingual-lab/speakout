...
| Path | Purpose |
| ---- | ------- |
| `supabase/schema.sql` | Reference schema (full table + RLS + functions) |
| `supabase/migrations/*.sql` | Timestamped migration files applied via CLI |
| `supabase/config.toml` | CLI config (auto-generated by `supabase init`) |

**Workflow for schema changes:**

1. Create a new migration file: `supabase/migrations/<YYYYMMDD>HHMMSS_description.sql`
2. Write the SQL (CREATE, ALTER, CREATE FUNCTION, etc.)
...
npx supabase link --project-ref <project-ref>       # links to remote project
```

**Pushing a migration:**

```bash
npx supabase db push    # applies pending migrations from supabase/migrations/
```

> Do **not** make schema changes directly in the Supabase dashboard. All changes go through migration files so they are version-controlled and reproducible.
