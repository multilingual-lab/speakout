...
- **STT:** Web Speech API (`ko-KR`), Chrome required. Both modes use interim results to reset a 10s silence timer, plus a hard max-duration cap (60s single / 120s continuous) as safety net against background noise. Single-result mode for practice/shadow; continuous mode (concatenated finals with interim preview) for monologue. No auto-restart on session end — avoids beep sounds on mobile Chrome caused by repeated `recognition.start()` calls.
