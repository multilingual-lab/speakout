...
- **STT:** Web Speech API (`ko-KR`), Chrome required. Both modes use interim results to reset a 10s silence timer, plus a hard max-duration cap (60s single / 120s continuous) as safety net against background noise. Single-result mode for practice/shadow; continuous mode (auto-restart on mobile, concatenated finals with interim preview) for monologue
