...
- **STT:** Web Speech API (`ko-KR`), Chrome required. `interimResults` disabled (causes beep noise on mobile). Single-result mode (30s safety timeout, browser auto-stops after one result) for practice/shadow; continuous mode (10s silence timer reset on each finalized phrase, auto-restart on mobile, concatenated finals) for monologue. Both modes have a hard max-duration cap (60s single / 120s continuous) against background noise.
